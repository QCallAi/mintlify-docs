...
3. Copy **OAuth Redirect URL** from AwazIndia to **Redirect URI** field on Gitlab page.
...
7. Copy the **Application ID** value from the following page to the `Client ID` field on AwazIndia.
8. Copy the **Secret** value from the same page to the `Client Secret` field on AwazIndia.
